		...
		if (!sc_Sport.isDisplayed()
			...
			for(int i=0;i<classements.size();i++){
				...
				if(!classements.get(i).isDisplayed()){
					log.debug("Erreur dans l'affichage des resultats");
					Assert.fail();
				}
			}
			...
			if(!classement.isDisplayed()){
				...
				switchpage.testerPage(driver, classement, "sports.orange.fr", "la page renvoyé par ce lien n'est pas convenable");
			}
