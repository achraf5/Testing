	...
	public SC_GuideTV(WebDriver driver, String xpath) throws InterruptedException{
		...
		if(!sc_GuideTv.isDisplayed() || !sc_GuideTv.getAttribute("class").equalsIgnoreCase("resultBlock shortcut shortcutGuideTv")){
			...
			if(!lienTtProgTele.isDisplayed() || !lienTtProgTele.getText().equalsIgnoreCase("toute la programmation télé")){
				System.out.println("le lien toute la programmation télé n'est pas affiché");
				Assert.fail();
			}
			else{
			...
			for(int i=0;i<bloc.size();i++){
				...
				for(int j=0;j<chaines.size();j++){
					...
					if(!logoCH.isDisplayed()){
						System.err.println("l'image du channel n'est pas affich�");
						Assert.fail();
					}else{
						switchpage.testerPage(driver, logoCH, "programme-tv.orange.fr", "la page renvoy� par ce lien n'est pas convenable au niveau du test "+i+","+j);
					}
					...
					if(!nom_prog.isDisplayed()){
						...
						switchpage.testerPage(driver, nom_prog, "programme-tv.orange.fr", "la page renvoy� par ce lien n'est pas convenable au niveau du test "+i+","+j);
					}
				}
			}
