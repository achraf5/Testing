	...
	public SC_ChaineTV(WebDriver driver, String xpath) throws InterruptedException{
		...
		if(!sc_prog_tv.isDisplayed()){
			...
			if(contentTvChannel.size() < 0 || contentTvChannel.size() >= 4){
				log.debug("le nombre de resultats programme tv n'est pas compatible a la condition");
				Assert.fail();
			}
			for(int i=0;i<contentTvChannel.size();i++){
				...
				if(i==(contentTvChannel.size()-1)){
					switchpage.testerPage(driver, contentTvChannel.get(i), "replay.orange.fr", "Erreur dans la page renvoye par le lien");
					//test sur l'image
					if(!img.isDisplayed()){
						log.debug("l'image n'est pas affiche");
						Assert.fail();
					}
					
					//test sur le lien
					if(!lien.isDisplayed()){
						log.debug("le lien n'est pas affich�");
						Assert.fail();
					}
				}else{
					switchpage.testerPage(driver, contentTvChannel.get(i), "programme-tv.orange.fr", "Erreur dans la page renvoye par le lien"+i);
					//test sur l'image
					if(!img.isDisplayed()){
						log.debug("l'image n'est pas affiche");
						Assert.fail();
					...
					//test sur le lien
					if(!lien.isDisplayed()){
						log.debug("le lien n'est pas affiche");
						Assert.fail();
					}
				}
				...
				if(!lienFooter.isDisplayed()){
					...
					switchpage.testerPage(driver, lienFooter, "programme-tv.orange.fr", "le lien toute la programation tele nous ne renvoie pas vers la bonne page");
				}
			}
